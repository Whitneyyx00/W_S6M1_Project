...
export default function Programmers() {
  ...
  const [programmers, setProgrammers] = useState(listOfAwesome); // STEP 2: Initialized with listOfAwesome
  const [featuredId, setFeaturedId] = useState(null); // STEP 2: Initialized with null or an empty string
  ...
  const getNameOfFeatured = () => {
    ...
    const featuredDev = programmers.find(dev => dev.id === featuredId); // Find the programmer by ID
    return featuredDev ? featuredDev.name : ''; // Return the name if found
  };
